...
public void OnEntityDestroyed(int entity) {
	...
	char buffer[512];
	if (!TF2CustAttr_GetString(weapon, "energy ring damage radius on destroy", buffer, sizeof(buffer))) {
		return;
	}
	
	float flKillDamage = ReadFloatVar(buffer, "damage", 60.0);
	float flKillRadius = ReadFloatVar(buffer, "radius", 30.0);
	...
	while ((rangeEntity = FindEntityInSphere(rangeEntity, vecDestroy, flKillRadius)) != -1) {
		if (IsEntityCombatCharacter(rangeEntity) && rangeEntity != owner) {
			SDKHooks_TakeDamage(rangeEntity, entity, owner, flKillDamage,
